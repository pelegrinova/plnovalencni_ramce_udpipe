...
data_z_webu = requests.post('http://lindat.mff.cuni.cz/services/udpipe/api/process', data={'tokenizer': "", 'tagger': "", 'parser': ""}, files={"data": open("nazev_textu_ke_zpracovani.txt", encoding="UTF-8")}) # odešle požadavek na web
...
def hledani_predikatu(veta): 
    ...
    return veta_hotove_predikaty, veta_predikaty_lemma, plnovalencni_ramce_prazdne
...
for veta in parsovani:
    ...
    # hledá predikáty
    veta_id_predikaty, veta_lemma_predikaty, pozice_do_ramcu = hledani_predikatu(veta)
...
with open("frekvence_rámců.csv", "wt") as csvfile: 
    ...
    vysledek_frekvence.writerow(["typ rámce", "frce"])
    for polozka in serazene_frce:
        ramec = ramec_k_zapisu(polozka[0])
        vysledek_frekvence.writerow([ramec, polozka[1]])
...
for x, lemmata_veta in enumerate(lemma_predikaty):
    for y, lemma in enumerate(lemmata_veta):
        lemma_a_jeho_ramec.append((lemma, plnovalencni_ramce[x][y]))  
...
with open("lemma_a_rámec.csv", "wt") as csvfile:
    ...
    vysledek_sparovani.writerow(["lemma", "rámec"])
    for polozka in lemma_a_jeho_ramec:
        ramec = ramec_k_zapisu(polozka[1])
        vysledek_sparovani.writerow([polozka[0], ramec])
